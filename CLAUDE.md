...
01-song-title/
    ...
    midi/                   ← Generated (for easy DAW import)
        ...
        vocal-melody.mid
        [etc...]
    .generated/             ← Generated (intermediate files, don't edit)
...
**You only edit: `song.yaml`**  
**Everything else is generated by: `python3 scripts/generate_song.py song-dir/`**
...
- Can be: `D2 D2 D2 D2` (4 quarters as eighths) = 2+2+2+2 = 8 ✓
- Can be: `D4 D4` (2 halves) = 4+4 = 8 ✓
- Cannot be: `D2 D2 D2 D2 D2` = 2+2+2+2+2 = 10 ✗
...
You write **ONE FILE**: `song.yaml`  
Everything else is **GENERATED**: `python3 scripts/generate_song.py song-dir/`
...
**Now (DRY, validated):**
- Edit `song.yaml` (contains everything)
- Run generator
- **Result:** All files guaranteed consistent, validated, correct

### The Complete Workflow (3 Steps)

#### Step 1: Write song.yaml

Create `song.yaml` with ALL song content in one place:
...
```bash
python3 scripts/generate_song.py song-directory/
```
...
- `midi/*.mid` - MIDI files ready for DAW import
...
- ❌ Manually write complete ABC files
- ❌ Edit generated ABC files directly (edit sections instead)
- ❌ Count bars yourself (let tools validate)
- ❌ Create lyrics.txt and chords.txt without YAML versions
- ❌ Skip validation steps

### DO:

- ✅ Use scripts/section_tools.py to generate templates
- ✅ Edit individual section ABC files (small, verifiable)
- ✅ Run validators after every change
- ✅ Keep lyrics.yaml and chords.yaml as source of truth
- ✅ Let build_song.py assemble complete files
...
3. **Map the narrative arc** - Opening awakening → confrontation → resolution
...
All complete song MIDI files are copied to `midi/` in the song root for easy import:

```bash
# To import a song into Bitwig/Ableton:
# Just drag the entire midi/ folder into your DAW

cd stone-gospel-rising/02-blood-on-the-capitol-steps/midi/
